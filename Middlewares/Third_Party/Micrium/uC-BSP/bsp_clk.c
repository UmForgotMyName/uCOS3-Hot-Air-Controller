...
void  BSP_ClkInit (void)
{
    ...
    RCC_OscInit.OscillatorType      = RCC_OSCILLATORTYPE_MSI;   /* MSI = internal default 4MHz internal RC oscillator   */
    RCC_OscInit.MSIState            = RCC_MSI_ON;
    RCC_OscInit.MSIClockRange       = RCC_MSIRANGE_6;
    ...
    RCC_OscInit.PLL.PLLN            = 40u;                      /* f(VCO clock) = (MSI / PLLM) * PLLN = 160MHz          */
    RCC_OscInit.PLL.PLLR            =  2u;                      /* PLLCLK       = f(VCO clock) / PLLR =  80MHz          */
    RCC_OscInit.PLL.PLLP            =  7u;
    RCC_OscInit.PLL.PLLQ            =  4u;
    ...
    RCC_ClkInit.ClockType     = (RCC_CLOCKTYPE_SYSCLK  |
                                 ...
                                 RCC_CLOCKTYPE_PCLK1   |
                                 RCC_CLOCKTYPE_PCLK2);
    RCC_ClkInit.SYSCLKSource   = RCC_SYSCLKSOURCE_PLLCLK;       /* SYSCLK  = PLLCLK = 80MHz                             */
    RCC_ClkInit.AHBCLKDivider  = RCC_SYSCLK_DIV1;               /* HCLK    = AHBCLK = PLLCLK / AHBPRES(1) = 80MHz.      */
    RCC_ClkInit.APB1CLKDivider = RCC_HCLK_DIV1;                 /* APB1CLK = AHBCLK / APB1DIV(1)          = 80MHz.      */
    RCC_ClkInit.APB2CLKDivider = RCC_HCLK_DIV1;                 /* APB2CLK = AHBCLK / APB2DIV(1)          = 80MHz.      */
